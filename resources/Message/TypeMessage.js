...
const TypeMessage = (props) => {
  ...
  const messageImage= (e)=> {
    ...
    Object.values(files).map((item, key)=> {
        ...
        console.log(formData.get("img-"+key))
  ...
  useEffect(()=> {
      listPreviewImg.map((item, key)=> setPi(prev=> ([...prev, {i: URL.createObjectURL(item), d: item.lastModified, k: key}])))
      return ()=> pi.map(item=> setListPreviewImg(prev=> ([...prev, URL.revokeObjectURL(item.i)])))
  }, [listPreviewImg])
  return (
    <div className="pr4" style={{display: "flex",width: "100%", flexDirection: "column", justifyContent: "space-evenly", alignItems: "center", position: "absolute", bottom: 0, left: 0, backgroundColor: "#fff", height: props.previewImg=== false ? 60 : 170}}>
        {
            props.previewImg=== true && <div className="pi6" style={{width: "94%", height: 100, overflowX: "auto", display: "flex", alignItems: "center", gap: 10, overflowY: "hidden"}}>
                {
                    pi?.length> 0 && pi.map((item, key)=> <div key={key} className="dk2" style={{width: 48, height: 48, minWidth: 48, minHeight: 48,padding: "0 10px", position: "relative"}} data-img-preview={`img-${parseInt(key)+ 1}`}>
                        ...
                        <div className="cs3" style={{width: 26, height: 26, borderRadius: "50%", display: "flex", justifyContent: 'center',alignItems: "center", border: "1px solid #dad5d5", backgroundColor: "#fff", position: "absolute", top: 0, right: 0, zIndex: 99, transform: "translate(10px, -10px)", cursor: "pointer"}} onClick={()=> setListPreviewImg(listPreviewImg?.filter((item2)=> item2.lastModified != item.d))}><CloseIcon style={{width: 20, height: 20}} /></div>
                    </div>)
                }
            </div>
        }
        ...
        <div className="vj3" style={{display: "flex", flexDirection: "row", alignItems: "center", gap: 10, width: "100%", padding: 5, backgroundColor: "#fff"}}>
            ...
            <div className="wi2" style={{width: "82%", height: 40, display: "flex", alignItems: "center"}}>
